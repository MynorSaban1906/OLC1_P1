...
class Lowercase(Function):
    ...
    def interpreter(self, tree, table):
        ...
        simbolo = table.getTabla("lowercase##param1") # crea una variable con el nombre complicado, algo que nunca vendra

        if simbolo ==None:
            return Exceptions("Semantico", "No se encontro el parametro de Lowercase ", self.getRow(),self.getColumn())

        if simbolo.getType() != Type.CADENA: # si no es igual al Type cadena entraria en un error
            return Exceptions("Semantico", "No se puede usar Lowercase  en  Type " +simbolo.getType() , self.getRow(),self.getColumn())
    

        self.setType(simbolo.getType()) # se pasa el Type de dato el cual siempre seria Type cadena
        
        return simbolo.getValue().lower() # se devuelve el valor en solo mayusculas
